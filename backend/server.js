...
// Set the frontend URL dynamically based on the environment
const allowedOrigin = isVercel ? `https://${process.env.VERCEL_URL}` : process.env.FRONTEND_URL || 'http://localhost:3000';
...
// Use a dynamic port if in Vercel, else default to a local port for development
const port = isVercel ? undefined : process.env.PORT || 3001;
app.listen(port, () => {
    console.log(`Server is running${isVercel ? ' on Vercel' : ' locally on port ' + port}`);
});
